...
Usage (on csns):
    ...
    python gen_eigenvector.py --cfg 2000 --Ne 8 --tslice-list 0,1 --backend numpy --no-save
    # production
    python gen_eigenvector.py --cfg 2000 --Ne 120 --tslices 128 --backend numpy \\
        --out out/2000.eigenvector.npy --out-eval out/2000.eigenvalue.npy
"""
...
DATA = "/dg_hpc/LQCD/DATA/clqcd_nf2_clov_L16_T128_b2.0_ml-0.05766_sn2_srho0.12_gg5.65_gf5.2_usg0.780268_usf0.949104"


def parse_args():
    p = argparse.ArgumentParser()
    p.add_argument("--cfg", default="2000")
    p.add_argument("--gauge-dir", default=f"{DATA}/00.cfgs/")
    p.add_argument("--gauge-prefix", default="clqcd_nf2_clov_L16_T128_b2.0_xi5_ml-0.05766_cfg_")
    p.add_argument("--latt-size", default="16,16,16,128", help="Lx,Ly,Lz,Lt")
    p.add_argument("--Ne", type=int, default=120)
    p.add_argument("--tol", type=float, default=1e-9)
    p.add_argument("--nstep", type=int, default=10)
    p.add_argument("--rho", type=float, default=0.12)
    p.add_argument("--tslices", type=int, default=1, help="number of timeslices to solve (starting at t=0)")
...
def main():
    ...
    gauge = GaugeFieldIldg(args.gauge_dir + args.gauge_prefix, ".lime", [Lt, Lz, Ly, Lx, Nd, Nc, Nc])
