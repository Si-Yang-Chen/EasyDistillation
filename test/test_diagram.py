...
class TestDiagram(unittest.TestCase):
    def setUp(self):
        ...
        from lattice.flavor_structure import Tag
        ...
        rows_list_tmp = hadron_little_group_projection([D_star_p2, D_meson_p1], "E", 0, parity=None)
        if not rows_list_tmp:
            print("no projection")
        else:
            # print(D_star_p2, D_meson_p1, len(rows_list_tmp))
            rows_list.extend(rows_list_tmp)
        rows_list_tmp = hadron_little_group_projection([D_star_p2, D_meson_p1], "E", 1, parity=None)
        if not rows_list_tmp:
            print("no projection")
        else:
            # print(D_star_p2, D_meson_p1, len(rows_list_tmp))
        ...
        chi_c1 = [
            HadronIrrep("chi_c1(0)", [0, 0, 1], "E", None, Tag(0, 0))[0],
        ]
        charmed = [
        ...
        iso_vector = [
            ...
            (HadronFlavorStructure("uu") - HadronFlavorStructure("dd")) / sp.sqrt(2),
        ...
        hadrons_list1 = (
            ...
            # + [Hadron(rows_list_jpsi_omega[0] + 2 * rows_list_jpsi_omega[1], charm_light)]
        )
        hadrons_list2 = (
            []
            + [Hadron(row, hidden_charm) for row in chi_c1]
        ...
        # for idx, hadron in enumerate(hadrons_list):
        #     print(idx, hadron.irrep_row)

        correlator = gen_correlator([hadrons_list1, hadrons_list2])
        correlator = remove_disconneted_diagram(correlator, [Rf"S^c_\mathrm{{local}}"])

        # Create basic Diagram object
        self.diagram = [correlator[0][0].expand(), correlator[1][0].expand()]

    def test_transform(self):
        """Test Diagram transformation"""
        little_group = genLittleGroupIrrep([0, 0, 1], "E", None)
        for ele in little_group.keys():
            for j in range(len(self.diagram)):
                rotated = operator_transform(self.diagram[j], ele, time=0)
                expected = S(0)
                for i in range(len(self.diagram)):
                    expected += little_group[ele][i, j] * self.diagram[i]
                diff = simplify(expected - rotated)
                self.assertEqual(diff, 0)


class TestDiagram2(unittest.TestCase):
    def setUp(self):
        """Set up test fixtures"""
        # Create basic test objects
        from lattice.flavor_structure import Tag
        ...
        rows_list = []
        D_star_p2 = HadronIrrep("D_star", [0, 1, 1], "B_1", None, Tag(0, 0))
        D_meson_p1 = HadronIrrep("D", [0, -1, 0], "A_2", None, Tag(0, 0))
        rows_list_tmp = hadron_little_group_projection([D_star_p2, D_meson_p1], "E", 0, parity=None)
        if not rows_list_tmp:
            print("no projection")
        ...
        chi_c1 = [
            HadronIrrep("chi_c1(0)", [0, 0, 1], "E", None, Tag(0, 0))[0]
            + HadronIrrep("chi_c1(0)", [0, 0, 1], "E", None, Tag(0, 0))[1],
        ]
        charmed = [
            HadronFlavorStructure("dc"),
            HadronFlavorStructure("uc"),
        ...
        # Create basic Diagram object
        self.diagram = [sp.simplify(correlator[0][0].expand()), sp.simplify(correlator[1][0].expand())]
