...
| --- | --- | --- | --- | --- |
| ENS-01 | done | 8-cfg localized VSV/PSV/PSP/overlap 输入 coverage 与 endpoint 兼容性审计 | Kunshan read-only preflight；VSV/PSP=18 source times、PSV=72；四类输入 8/8；现有 VSV 不能覆盖 full-time J4 endpoint | 新 storage 到位后重跑 hash-enabled audit |
| ENS-02 | in-progress | 8-cfg Wilson `J4×J4` production input/result contract | full-time audit、source/execution record、atomic result schema 和 one-config wrapper 已完成；无真实 full-time VSV | storage 到位后运行 audit，绑定 artifacts 和 resource pilot |
