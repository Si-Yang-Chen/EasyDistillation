...
class Current:
    ...
    def compute_elemental(
        ...
        # Preserve the lightweight adapter contract used by the localized-
        # blending tests for generators that expose only calc_all(). The full
        # current-flow contract is selected when Ne/usedNe/Np/usedNp metadata
        # is available.
        if not hasattr(gen, "Ne") and not hasattr(gen, "usedNe"):
            results = calc_all(t)
            if not isinstance(results, Mapping):
                raise TypeError("generator.calc_all(t) must return a mapping")
            if self.elemental_key not in results:
                raise TypeError(f"generator.calc_all(t) has no {self.elemental_key!r} elemental")
            return {
                "operator": self._operator(),
                "terms": tuple(x.as_dict() for x in self.terms),
                "elemental": results[self.elemental_key],
                "all_elementals": results,
                "time": t,
            }
